...
async function buildSessionIndex(filePath, projectDir) {
    try {
...
export async function buildAllSessionIndexes() {
    const allFiles = [];
    try {
    ...
    for (let i = 0; i < allFiles.length; i += CONCURRENCY) {
        ...
        const results = await Promise.all(batch.map(f => buildSessionIndex(f.filePath, f.projectDir)));
...
export function searchSessions(sessions, query, maxResults = 50) {
    if (!query.trim())
        return [];
...
function extractSnippet(text, terms, contextChars = 60) {
